...
public class StudentAdvancedService extends ServiceImpl<StudentAdvancedMapper, StudentAdvanced> implements IStudentAdvancedService {
    ...
    public DataResponse judgeStudentAdvancedData(StudentAdvanced studentAdvanced){
        if(studentMapper.checkStudentId(studentAdvanced.getStudentId())==0)return CommomMethod.getReturnMessageError("不存在该学生");
        ...
        if(studentAdvanced.getAdvancedType()==AdvancedType.Volunteer&&studentAdvanced.getDuration()==null)return CommomMethod.getReturnMessageError("志愿类型活动时长不可为空");
